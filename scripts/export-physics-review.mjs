...
const lines = [
  ...
  `Audit confidence: structural **${bankAudit.confidence.structural}** (${bankAudit.structuralChecked} parts); numerical checks **${bankAudit.confidence.numerical}**, raw claim coverage **${bankAudit.confidence.numericalCoverage}** (${bankAudit.numericalClaimsVerified}/${bankAudit.numericalClaimsDetected} claims verified = ${bankAudit.numericalCoveragePercent === null ? "n/a" : `${bankAudit.numericalCoveragePercent.toFixed(1)}%`}; ${bankAudit.numericalClaimsParsed} parsed, ${bankAudit.numericalClaimsUnresolved} unresolved, ${bankAudit.numericalClaimErrors} claim errors; ${bankAudit.numericalVerified}/${bankAudit.numericalChecks} equation checks passed, ${bankAudit.numericalErrors} arithmetic errors); derived-result coverage ${bankAudit.derivedClaimsVerified}/${bankAudit.derivedClaimsDetected} verified = ${bankAudit.derivedCoveragePercent === null ? "n/a" : `${bankAudit.derivedCoveragePercent.toFixed(1)}%`} (${bankAudit.derivedClaimsUnresolved} unresolved); dimensional checks **${bankAudit.confidence.dimensional}**, bank coverage **${bankAudit.confidence.dimensionalCoverage}** (${bankAudit.dimensionalVerified}/${bankAudit.dimensionalClaimsDetected} candidates checked = ${bankAudit.dimensionalCoveragePercent === null ? "n/a" : `${bankAudit.dimensionalCoveragePercent.toFixed(1)}%`}; ${bankAudit.dimensionalErrors} errors; symbolic ${bankAudit.symbolicDimensionalVerified}/${bankAudit.symbolicDimensionalChecks} verified, ${bankAudit.symbolicDimensionalUnresolved} unresolved); scheme↔answer coverage **${bankAudit.confidence.schemeAnswerCoverage}** (${bankAudit.schemeAnswerMatchedQuantities} matched, ${bankAudit.schemeAnswerUnmatchedSchemeQuantities} unmatched scheme, ${bankAudit.schemeAnswerUnmatchedAnswerQuantities} unmatched worked-answer, ${bankAudit.schemeAnswerAmbiguousPairings} ambiguous; ${bankAudit.schemeAnswerCoveragePercent === null ? "n/a" : `${bankAudit.schemeAnswerCoveragePercent.toFixed(1)}%`} pairing coverage). Physics-law checks: ${bankAudit.physicsRuleVerified}/${bankAudit.physicsRuleChecks} passed (${bankAudit.physicsRuleDetected} cues, ${bankAudit.physicsRuleUnresolved} unresolved). Plausibility checks: ${bankAudit.plausibilityChecks}, ${bankAudit.plausibilityWarnings} warnings. Priority queue: ${bankAudit.numericalReviewQueue.length} items (${Object.entries(bankAudit.numericalReviewQueueCounts).map(([priority, count]) => `${priority} ${count}`).join(", ")}). ${bankAudit.manualReviewRequired} parts require manual numerical review.`,
  "",
...
for (const row of rows) {
  ...
  for (const part of row.question.parts) {
    ...
    lines.push(part.prompt, "", `Marks: ${part.marks}`, "", ...part.markScheme.map((point) => `- ${point}`),
      ...
      `Deterministic numerical audit: ${numerical?.status ?? "unresolved"}; raw claims ${numerical?.claimsVerified ?? 0}/${numerical?.claimsDetected ?? 0}, derived ${numerical?.derivedClaimsVerified ?? 0}/${numerical?.derivedClaimsDetected ?? 0} verified (${numerical?.derivedClaimsUnresolved ?? 0} unresolved); equations ${numerical?.arithmeticVerified ?? 0}/${numerical?.arithmeticChecks ?? 0}; dimensions ${numerical?.dimensionalVerified ?? 0}/${numerical?.dimensionalCoverage?.claimsDetected ?? 0} candidates checked; symbolic dimensions ${numerical?.symbolicDimensionalVerified ?? 0}/${numerical?.symbolicDimensionalChecks ?? 0}; scheme↔answer ${numerical?.schemeAnswerMatchedQuantities ?? 0} matched (${numerical?.schemeAnswerCoverage?.unmatchedSchemeQuantities ?? 0} scheme-only, ${numerical?.schemeAnswerCoverage?.unmatchedAnswerQuantities ?? 0} answer-only, ${numerical?.schemeAnswerCoverage?.ambiguousPairings ?? 0} ambiguous); Physics laws ${numerical?.physicsRuleVerified ?? 0}/${numerical?.physicsRuleChecks ?? 0}; plausibility warnings ${numerical?.plausibilityWarnings ?? 0}. ${numerical?.issues.length ? `Issues: ${numerical.issues.map((issue) => `${issue.kind} (${issue.detail})`).join("; ")}` : "No deterministic issue; still requires human subject review."}`,
      "",
...
console.log(JSON.stringify({ questions: rows.length, replacements: rows.filter((row) => row.question.id.startsWith("cnt:question:physics-quality-")).length, approvalsCreated: 0,
  ...
  derivedClaimsDetected: bankAudit.derivedClaimsDetected, derivedClaimsParsed: bankAudit.derivedClaimsParsed, derivedClaimsVerified: bankAudit.derivedClaimsVerified, derivedClaimsUnresolved: bankAudit.derivedClaimsUnresolved, derivedClaimErrors: bankAudit.derivedClaimErrors, derivedCoveragePercent: bankAudit.derivedCoveragePercent,
  dimensionalClaimsDetected: bankAudit.dimensionalClaimsDetected, dimensionalChecks: bankAudit.dimensionalChecks, dimensionalVerified: bankAudit.dimensionalVerified, dimensionalErrors: bankAudit.dimensionalErrors, dimensionalUnresolved: bankAudit.dimensionalUnresolved, dimensionalCoveragePercent: bankAudit.dimensionalCoveragePercent, symbolicDimensionalChecks: bankAudit.symbolicDimensionalChecks, symbolicDimensionalVerified: bankAudit.symbolicDimensionalVerified, symbolicDimensionalErrors: bankAudit.symbolicDimensionalErrors, symbolicDimensionalUnresolved: bankAudit.symbolicDimensionalUnresolved,
  schemeAnswerCandidates: bankAudit.schemeAnswerCandidates, schemeAnswerChecks: bankAudit.schemeAnswerChecks, schemeAnswerVerified: bankAudit.schemeAnswerVerified, schemeAnswerErrors: bankAudit.schemeAnswerErrors, schemeAnswerUnresolved: bankAudit.schemeAnswerUnresolved, schemeAnswerMatchedQuantities: bankAudit.schemeAnswerMatchedQuantities, schemeAnswerUnmatchedSchemeQuantities: bankAudit.schemeAnswerUnmatchedSchemeQuantities, schemeAnswerUnmatchedAnswerQuantities: bankAudit.schemeAnswerUnmatchedAnswerQuantities, schemeAnswerAmbiguousPairings: bankAudit.schemeAnswerAmbiguousPairings, schemeAnswerCoveragePercent: bankAudit.schemeAnswerCoveragePercent,
  physicsRuleDetected: bankAudit.physicsRuleDetected, physicsRuleChecks: bankAudit.physicsRuleChecks, physicsRuleVerified: bankAudit.physicsRuleVerified, physicsRuleErrors: bankAudit.physicsRuleErrors, physicsRuleUnresolved: bankAudit.physicsRuleUnresolved, plausibilityChecks: bankAudit.plausibilityChecks, plausibilityWarnings: bankAudit.plausibilityWarnings, numericalReviewQueueItems: bankAudit.numericalReviewQueue.length, numericalReviewQueueCounts: bankAudit.numericalReviewQueueCounts, manualReviewRequired: bankAudit.manualReviewRequired, confidence: bankAudit.confidence,
  output: out }));
