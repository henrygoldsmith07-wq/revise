...
export interface WorkingAnalysis { modelSteps: string[]; steps: StudentStep[]; firstIncorrect: FirstIncorrect | null; consistentWithModel: boolean }
...
export interface WorkedSolutionValidation { status: WorkedSolutionValidationStatus; modelSteps: string[]; markSchemePoints: number; coveredMarkSchemePoints: number; markSchemeCoverage: number; issues: WorkedSolutionIssue[] }
export interface WorkedSolutionAuditIssue extends WorkedSolutionIssue { questionId: string; partId: string }
export interface WorkedSolutionAudit { status: WorkedSolutionValidationStatus; questionCount: number; partCount: number; passedParts: number; errors: number; warnings: number; issues: WorkedSolutionAuditIssue[] }
...
function canonicaliseAuthoredNumericNotation(text: string): string {
  ...
  out = out.replace(/(?:√|\bsqrt\s*)\s*([⟨(\[])([^⟩)\]]+)[⟩)\]]/gi, (match, _open: string, radicand: string) => /[a-z]/i.test(radicand) ? " " : match);
  out = out.replace(/(-?\d+(?:\.\d+)?)\s*(?:√|\bsqrt\s*)\s*\(?\s*(-?\d+(?:\.\d+)?)\s*\)?/gi, (_match, coefficient: string, radicand: string) => {
    const value = Number(coefficient) * Math.sqrt(Number(radicand)); return Number.isFinite(value) ? String(value) : _match;
  });
  out = out.replace(/(?:√|\bsqrt\s*)\s*\(?\s*(-?\d+(?:\.\d+)?)\s*\)?/gi, (_match, raw: string) => {
    const value = Math.sqrt(Number(raw)); return Number.isFinite(value) ? String(value) : _match;
  });
...
function authoredNumericEquivalent(expected: string, actual: string): boolean {
  return numericEquivalent(expected, actual) || numericEquivalent(canonicaliseAuthoredNumericNotation(expected), canonicaliseAuthoredNumericNotation(actual));
}
export function validateWorkedSolution(part: QuestionPart): WorkedSolutionValidation {
  ...
  for (const [pointIndex, point] of markScheme.entries()) {
    ...
    for (const bad of findUnbalancedEquations(`${point} ${modelAnswer}`)) issues.push({ kind: "unbalanced-equation", severity: "warning", pointIndex, detail: `Equation does not balance: ${bad}` });
    if (numericExpected && !numericMatches) { issues.push({ kind: "numeric-mismatch", severity: "error", pointIndex, detail: `Mark-scheme point ${pointIndex + 1} expects a numerical result that is not present in the model answer: ${point}` }); continue; }
  ...
  return { status: errors ? "fail" : warnings ? "review" : "pass", modelSteps, markSchemePoints: markScheme.length, coveredMarkSchemePoints, markSchemeCoverage: markScheme.length ? Math.round((coveredMarkSchemePoints / markScheme.length) * 100) : 0, issues };
}
export function validateWorkedSolutions(questions: readonly Question[]): WorkedSolutionAudit {
  const issues: WorkedSolutionAuditIssue[] = []; let partCount = 0; let passedParts = 0;
  for (const question of questions) for (const part of question.parts ?? []) {
    partCount++; const validation = validateWorkedSolution(part); if (validation.status === "pass") passedParts++;
...
export function firstIncorrectStep(part: QuestionPart, answer: string): WorkingAnalysis {
  const modelSteps = modelStepsForPart(part); const steps = splitSteps(answer).map((text, index) => ({ index, text })); const contradiction = contradictoryWorkingStep(answer);
  if (!steps.length) return { modelSteps, steps, firstIncorrect: modelSteps.length ? { stepIndex: 0, studentStep: "", expected: modelSteps[modelSteps.length - 1]!, similarity: 0, reason: "working-runs-out" } : null, consistentWithModel: false };
  const covered = new Array<boolean>(modelSteps.length).fill(false); let nextModel = 0;
  for (const step of steps) {
    if (nextModel >= modelSteps.length) { if (contradiction !== null) return { modelSteps, steps, consistentWithModel: false, firstIncorrect: { stepIndex: contradiction, studentStep: steps[contradiction]?.text ?? answer, expected: "Keep numerical statements consistent; identify any corrected or abandoned working.", similarity: 0, reason: "contradictory-working" } }; continue; }
    ...
    if (futureIndex >= 0) return { modelSteps, steps, consistentWithModel: false, firstIncorrect: { stepIndex: step.index, studentStep: step.text, expected: modelSteps[nextModel]!, similarity: forward, reason: "missing-expected-step" } };
    if (contradiction !== null && contradiction === step.index) return { modelSteps, steps, consistentWithModel: false, firstIncorrect: { stepIndex: step.index, studentStep: step.text, expected: modelSteps[nextModel]!, similarity: 0, reason: "contradictory-working" } };
    if (forward < STEP_THRESHOLD) return { modelSteps, steps, consistentWithModel: false, firstIncorrect: { stepIndex: step.index, studentStep: step.text, expected: modelSteps[nextModel]!, similarity: forward, reason: "content-mismatch" } };
  }
...
export function consistentWithModel(part: QuestionPart, answer: string): boolean { return firstIncorrectStep(part, answer).consistentWithModel; }
...
function markKindCounts(part: QuestionPart, marked: MarkedPart): Pick<AttemptWorkingEvidence, "methodMarksAwarded" | "accuracyMarksAwarded" | "followThroughMarksAwarded" | "unitMarksAwarded" | "precisionMarksAwarded"> {
  ...
  return { methodMarksAwarded: count("method"), accuracyMarksAwarded: count("accuracy"), followThroughMarksAwarded: count("follow-through"), unitMarksAwarded: count("unit"), precisionMarksAwarded: count("precision") };
}
export function buildWorkingEvidence(part: QuestionPart, marked: MarkedPart, answer: string): AttemptWorkingEvidence {
  ...
  const firstErrorKind: WorkingErrorKind = contradiction !== null ? "contradictory-working" : diagnosis.kind;
  const counts = markKindCounts(part, marked);
  return { partId: part.id, firstIncorrectStep: contradiction ?? analysis.firstIncorrect?.stepIndex ?? diagnosis.firstErrorIndex, firstErrorKind, consistentWithModel: analysis.consistentWithModel && contradiction === null, ...counts, errorCarriedForward: counts.followThroughMarksAwarded > 0 && counts.accuracyMarksAwarded === 0 };
}
