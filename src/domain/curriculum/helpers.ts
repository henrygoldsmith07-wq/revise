  ...
  specs.forEach((unitSpec, unitIndex) => {
    ...
    for (const topicSpec of unitSpec.topics) {
      ...
      const specPoints: SpecPoint[] | undefined = topicSpec.specPoints?.map((sp, i) => ({
        ...
        ...((sp as SpecPoint).specVersion ?? topicSpec.specVersion) ? { specVersion: (sp as SpecPoint).specVersion ?? topicSpec.specVersion } : {},
      }));
      topics.push({
        id: topicId,
        subjectId,
        ...
        order: order++,
        ...(topicSpec.specRef ? { specRef: topicSpec.specRef } : {}),
        ...(topicSpec.specVersion ? { specVersion: topicSpec.specVersion } : {}),
        ...
        ...(topicSpec.aos ? { aos: topicSpec.aos } : {}),
