...
const questions: QuestionSpec[] = [
  {
    ...
    parts: [
      ...
      qualityPart({ topic: "capacitance", point: 2, demand: "calculation", family: "capacitance:clinical", context: "capacitance:clinical:stored-energy", prompt: "A 180 μF capacitor is charged to 2.4 kV. Calculate the energy available for the pulse and state the unit.", marks: 3, scheme: ["Use E = ½CV²", "C = 180 × 10⁻⁶ F and V = 2400 V", "E = 0.5 × 180 × 10⁻⁶ × 2400² = 518 J"], answer: "E = ½CV² = ½(180 × 10⁻⁶)(2400)² = 5.18 × 10² J, so about 518 J is stored.", move: "carry SI prefixes through a squared voltage calculation", calculationRules: [{ kind: "method", label: "energy", expected: 518, method: { operator: "*", operands: ["C", "V"] } }, { kind: "accuracy", label: "E", expected: 518 }, { kind: "unit", label: "E", expected: 518, unitAliases: ["J"] }] }),
  ...
  },
  {
    ...
    parts: [
      ...
      qualityPart({ topic: "alternating-currents", point: 3, demand: "transfer", family: "alternating-currents:grid-filter", context: "alternating-currents:grid-filter:transmission", prompt: "A transmission line delivers 120 kW. Compare the cable loss at 20 kV and 200 kV if the cable resistance is 4.0 Ω. State the assumption used.", marks: 4, scheme: ["For fixed power I = P/V", "Currents are 6.0 A and 0.60 A", "P_loss = I²R gives 144 W and 1.44 W", "Ideal transformer changes voltage without changing frequency and power is treated as conserved"], answer: "At 20 kV, I = 120000/20000 = 6.0 A and loss = 6.0² × 4.0 = 144 W. At 200 kV, I = 0.60 A and loss = 0.60² × 4.0 = 1.44 W. This assumes ideal voltage transformation and the same cable resistance.", move: "link rms power, transformer action and resistive loss" }),
  ...
  },
  {
    ...
    parts: [
      ...
      qualityPart({ topic: "sports-physics", point: 3, demand: "transfer", family: "sports-physics:sprint", context: "sports-physics:sprint:landing", prompt: "A 70 kg athlete's vertical speed changes from −4.0 m s⁻¹ to zero. Compare the average landing force if a mat increases stopping time from 0.05 s to 0.20 s. Include weight in the net-force calculation.", marks: 4, scheme: ["Change in momentum is m(0 − (−4.0)) = 280 kg m s⁻¹ upward", "Average net force is Δp/Δt: 5600 N or 1400 N", "The ground force is net force plus weight: about 6290 N or 2080 N", "The longer time reduces the average force and injury risk"], answer: "The upward change in momentum is 70 × 4.0 = 280 kg m s⁻¹. Net average force is 280/0.05 = 5600 N without the mat and 280/0.20 = 1400 N with it. The ground force is net force + mg, giving about 6290 N and 2087 N respectively. Increasing stopping time reduces the average force.", move: "distinguish net force from contact force in a landing" }),
  ...
  },
  {
    ...
    parts: [
      ...
      qualityPart({ topic: "energy-environment", point: 2, demand: "calculation", family: "energy-environment:peak", context: "energy-environment:peak:efficiency", prompt: "A storage system takes 240 MWh from the grid and returns 180 MWh. Calculate its round-trip efficiency and the mean power returned over 3.0 h.", marks: 3, scheme: ["Efficiency = useful output/input = 180/240 = 0.75 = 75%", "Mean returned power = 180 MWh/3.0 h = 60 MW", "Energy not returned is dissipated or retained in the system"], answer: "Round-trip efficiency = 180/240 = 0.75, or 75%. If the 180 MWh is returned over 3.0 h, the mean output power is 60 MW.", move: "calculate efficiency and power from separate quantities", calculationRules: [{ kind: "accuracy", label: "efficiency", expected: 0.75 }, { kind: "precision", label: "efficiency", expected: 75, significantFigures: 2 }, { kind: "accuracy", label: "power", expected: 60 }, { kind: "unit", label: "power", expected: 60, unitAliases: ["MW"] }] }),
  ...
  },
  {
    ...
    verification: "unverified",
    parts: [
      qualityPart({ topic: "practical-investigations", point: 1, demand: "misconception", family: "practical-investigations:pendulum", context: "practical-investigations:pendulum:repeats", prompt: "A student repeats a timing measurement ten times and says this removes the timer's zero error. Explain which uncertainty repeats reduce and how a zero error should be checked.", marks: 3, scheme: ["Repeats and averaging reduce random uncertainty", "A fixed zero offset is systematic and remains in every result", "Check against a known interval or calibrate/reset the timer and record the correction"], answer: "Repeats reduce random scatter in the measured period but do not remove a fixed zero offset. Compare the timer with a known time signal or calibrate/reset it, then apply and document the correction.", move: "separate random variation from a shared systematic shift" }),
      qualityPart({ topic: "practical-investigations", point: 2, demand: "calculation", family: "practical-investigations:pendulum", context: "practical-investigations:pendulum:uncertainty", prompt: "A length is measured as 0.800 ± 0.002 m and a 20-cycle time as 35.80 ± 0.20 s. Calculate the period and its percentage uncertainty using addition of fractional uncertainties.", marks: 4, scheme: ["T = 35.80/20 = 1.790 s", "Fractional time uncertainty = 0.20/35.80", "Fractional length uncertainty = 0.002/0.800", "Percentage uncertainty in T is about 0.6%; quote T = 1.790 ± 0.011 s"], answer: "T = 35.80/20 = 1.790 s. The fractional uncertainty from timing is 0.20/35.80 = 0.00559 and from length is 0.002/0.800 = 0.00250. Adding gives 0.00809, or about 0.81%, so ΔT ≈ 0.014 s (T ≈ 1.790 ± 0.014 s). If only the period is required, state the adopted uncertainty convention explicitly.", move: "propagate stated measurement uncertainty and expose the convention", calculationRules: [{ kind: "accuracy", label: "period", expected: 1.79 }, { kind: "unit", label: "period", expected: 1.79, unitAliases: ["s"] }, { kind: "precision", label: "period", expected: 1.79, significantFigures: 4 }] }),
